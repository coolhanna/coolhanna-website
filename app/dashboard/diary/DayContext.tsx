...
import { dailyTaskGroups } from "./schedule-model";
...
export default function DayContext({ context, loading, error, today, evening, renderEntry, onReload, onCalendar }: DayContextProps) {
  const [showAllTasks, setShowAllTasks] = useState(false);
  const { due: tasks, upcoming, undated } = dailyTaskGroups(context?.open_tasks.entries || [], today);
  ...
  return <div className={styles.dailyContext}>
    ...
    <section className={styles.openTasksSection} aria-labelledby="open-tasks-title">
      <div className={styles.sectionHeading}><div><p className={styles.eyebrow}>오늘 일정과 날짜가 지난 일</p><h2 id="open-tasks-title">{evening ? "오늘 남은 일" : "오늘 챙길 일"}</h2></div>{context?.open_tasks.status === "available" && <span>{tasks.length}개</span>}</div>
      ...
      </> : <p className={styles.emptyState}>오늘 남은 일정은 없어요.</p> : !loading && <p className={styles.emptyState}>남아 있는 일을 확인하지 못했어요. 기록이 없다는 뜻은 아니에요.</p>}
      {context?.open_tasks.status === "available" && <button type="button" className={styles.textButton} onClick={onCalendar}>앞으로의 일정 {upcoming.length}개 · 날짜 미정 {undated.length}개 → 주간에서 보기</button>}
    </section>
